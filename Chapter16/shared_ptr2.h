...
public:
    ...
    shared_ptr2(T *tp, function<void (T*)> fp=nullptr):p(tp), ip(new int(1)), delFun(fp){}
    ~shared_ptr2();
    shared_ptr2(const shared_ptr2 &sp, function<void (T*)> fp = nullptr);
    shared_ptr2 &operator=(const shared_ptr2 &);
    shared_ptr2(shared_ptr2 &&sp, function<void (T*)> fp = nullptr);
    ...
    bool unique() const;
    int use_count() const;
    void reset(T* sptr = nullptr, function<void (T*)> fp = nullptr);

private:
    void destroy();
    T *p;
    int *ip;
    function<void (T*)> delFun;
};
...
template <typename T>
shared_ptr2<T>::shared_ptr2(const shared_ptr2 & sp, function<void (T *)> fp) : p(sp.p), ip(sp.ip){
	++*ip;
	if(fp == nullptr)
		delFun = sp.delFun;
	else
...
template <typename T>
shared_ptr2<T>::shared_ptr2(shared_ptr2 &&sp, function<void (T*)> fp):p(std::move(sp.p)), ip(std::move(sp.ip)){
    if(fp == nullptr)
		delFun = sp.delFun;
	else
		delFun = fp;
...
template <typename T>
void shared_ptr2<T>::reset(T * sptr, function<void (T *)> fp){
	destroy();
	p = sptr;
	if(p != nullptr)
		ip = new int(1);
